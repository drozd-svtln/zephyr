...
static int get_id(char *str, uint32_t *id)
{
	...
	if (parsed_id < 0 || parsed_id > UINT32_MAX ||
	    str == endptr || *endptr != '\0' || errno == ERANGE) {
		return -EINVAL;
	}
...
static int scmi_shell_base_version(const struct shell *sh, size_t argc, char **argv)
{
	...
	ret = scmi_base_get_version(&version);
	if (ret != 0) {
		shell_error(sh, "base proto get version failed (%d)", ret);
		return ret;
	}
