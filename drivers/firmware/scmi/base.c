...
DT_SCMI_PROTOCOL_DATA_DEFINE(0, SCMI_PROTOCOL_BASE, NULL, SCMI_BASE_PROTOCOL_SUPPORTED_VERSION);
#else /* CONFIG_ARM_SCMI_TRANSPORT_HAS_STATIC_CHANNELS */
DT_SCMI_PROTOCOL_DATA_DEFINE(0, SCMI_PROTOCOL_BASE, NULL);
...
struct scmi_msg_base_attributes_reply {
    int32_t status;
...
static int scmi_base_get_version(uint32_t *version)
{
...
static int scmi_base_attributes_get(struct scmi_msg_base_attributes_reply *attr)
{
	int ret;
...
union scmi_base_msgs_t {
	struct scmi_msg_base_attributes_reply attr;
...
int scmi_base_get_revision_info(struct scmi_revision_info *rev)
{
	...
	ret = scmi_base_get_version(&(version.raw));
	if (ret) {
		return ret;
	}
    ...
    if (msgs.attr.status != SCMI_SUCCESS) {
		return scmi_status_to_errno(msgs.attr.status);
	}
